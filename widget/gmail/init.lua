...
local gmail_count = wibox.widget {
  markup = '<span font="SF Pro Display Regular 12">Loading...</span>',
  align = 'left',
  valign = 'center',
  widget = wibox.widget.textbox
}
  ...
  awful.spawn.easy_async_with_shell(check_count, function(stdout)
    if tonumber(stdout) ~= nil then
      ...
      if tonumber(unread_count) > 1 then
        gmail_count.markup = '<span font="SF Pro Display Regular 14">You have ' .. unread_count .. ' unread emails!</span>'
      else
        gmail_count.markup = '<span font="SF Pro Display Regular 14">You have ' .. unread_count .. ' unread email!</span>'
      end

      -- Notify
      notify_new_email(unread_count)
    else
      gmail_count.markup = '<span font="SF Pro Display Regular 12">Check internet connection!</span>'
    end
  end)
end
