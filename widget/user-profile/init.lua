...
local PATH_TO_ICONS = HOME .. '/.config/awesome-floppy/widget/user-profile/icons/'
...
awful.spawn.easy_async_with_shell('whoami', function(out)
  ...
  profile_name.markup = '<span font="SF Pro Display Bold 24">' .. name ..'</span>' --out:sub(1,1):upper()..out:sub(2)
...
awful.spawn.easy_async_with_shell("cat /etc/os-release | awk 'NR==1'| awk -F " .. "'" .. '"' .. "'" .. " '{print $2}'", function(out)
  ...
  distroname = out:gsub('%\n','')
  distro_name.markup = '<span font="SF Pro Display Regular 12">' .. distroname ..'</span>'
end)

-- RUn once on startup or login
awful.spawn.easy_async_with_shell("uptime -p", function(out)
  uptime = out:gsub('%\n','')
  uptime_time.markup = '<span font="SF Pro Display Regular 10">' .. uptime ..'</span>'
end)
